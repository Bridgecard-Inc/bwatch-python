...
class subscriber:
    ...
    def __init__(self, URL, function):
        self.cache = {}
        self.remote_thread = RemoteThread(self, dbURL(URL), function)
...
def put(URL, msg):
    ...
    response = requests.put(dbURL(URL), data=to_post)
...
def patch(URL, msg):
    to_post = json.dumps(msg)
    response = requests.patch(dbURL(URL), data=to_post)
    if response.status_code != 200:
        raise FirebaseException(response.text)


def get(URL):
    response = requests.get(dbURL(URL))
    if response.status_code != 200:
        raise FirebaseException(response.text)
...
def push(URL, msg):
    to_post = json.dumps(msg)
    response = requests.post(dbURL(URL), data=to_post)
    if response.status_code != 200:
